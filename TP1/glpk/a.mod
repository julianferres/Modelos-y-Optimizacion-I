...
s.t. visitaIantesJ{i in CIUDADES, j in CIUDADES: i<>j}: -M*(1 - A[i,j]) <= U[j] - U[i];
s.t. visitaIantesJ_1{i in CIUDADES, j in CIUDADES: i<>j}: U[j] - U[i] <= M * A[i,j];

#Definición V_ijk:
s.t. ciudDistA{i in CIUDADES, j in CIUDADES, k in CIUDADES: i<>j and j<>k and i<>k} :2 * V[i,j,k]= A[i,j] + Y[i,j];
